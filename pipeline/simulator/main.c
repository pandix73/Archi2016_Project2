...
void ID(){
	...
	} else if (IDEX.opcode != j && IDEX.opcode != jal && IDEX.opcode != halt && IDEX.opcode != lui){ // use both rs and rt
		if(IDEX.rs == IDEX.rt){
			if(rsInEXDM){
...
void EX(){	
	if(IDEX.stall == 1){
		...
		EXDM.isNOP = 1;
		return;
	} else if (IDEX.fwd.happen && IDEX.opcode != bne && IDEX.opcode != beq && IDEX.opcode != bgtz){
